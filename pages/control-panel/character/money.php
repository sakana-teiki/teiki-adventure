<?php
  ...
  if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (!$result) {
      responseError(500); // 実行に失敗した場合は500(Internal Server Error)を返して処理を中断
    }
  }

  $PAGE_SETTING['TITLE'] = '所持金付与';
...
<section>
  ...
  <p>
    ENo.<?=$_POST['eno']?>に所持金が<?=$_POST['number']?>G付与されました。 
  </p>
</section>
<?php } ?>
...
<section>
  ...
  <form id="form" method="post">
    ...
    <section class="form">
      ...
      <div class="form-description">付与を行うキャラクターのENoを指定します。</div>
    ...
    <section class="form">
      <div class="form-title">付与額</div>
      <div class="form-description">付与する金額を指定します。</div>
      <input id="input-money" class="form-input" type="number" name="number" placeholder="金額">
    </section>
...
<script>
  ...
  $('#form').submit(function(){
    ...
    // 付与金額が入力されていない場合エラーメッセージを表示して送信を中断
    if (!inputMoney) {
      showErrorMessage('付与額が入力されていません。');
      return false;
    }
