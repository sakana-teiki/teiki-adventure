...
| target | INT? UNSIGNED | 通知の対象を表します。通知対象によって表す値は変わり、`announcement`ではお知らせのidを、`replied`では返信が行われた発言のidを、`new_arrival`では発言が行われたトークルームのRNoを、`faved`ではお気に入りを行ったキャラクターのENoを、`direct_message`ではダイレクトメッセージのidを表します。対象が存在しない通知タイプではNULLとなります。 |
...
| state | ENUM | アイテム送付の状態です。`trading`であれば許可待ち、`finished`であればアイテム送付完了、`cancelled_by_master`であれば送付元によってキャンセルされたことを、`cancelled_by_target`であれば送付元によってキャンセルされたことを表します。 |
