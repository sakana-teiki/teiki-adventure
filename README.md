...
## 諸注意
このプロジェクトは現在リリース直後となっています。足りない機能が多く、不具合が含まれている可能性が高いです。
...
⚠️このプロジェクトを利用したサイトを公開する前にはdocuments/公開前のチェックリスト.mdを確認してください。
...
4. MySQL(MariaDB)のコンソールに入るので以下のコマンド群を順に実行します。MySQL(MariaDB)内にパスワードが`dbpassword`のユーザー`teiki`と、`teiki_adventure`というデータベースが作成され、ユーザー`teiki`にデータベース`teiki_adventure`に対する全権限が付与されます。
...
## ドキュメント
本プロジェクトに関してのドキュメントをdocumentsフォルダにまとめていますので、必要に応じて参照してください。

|内容|ファイル|
|---|---|
|Mac版セットアップ手順|documents/Mac版セットアップ.md|
|ゲーム仕様|documents/ゲーム仕様.md|
|データベースの各カラム仕様|documents/データベースの各カラム仕様.md|
|各ディレクトリ・ファイルの役割|documents/各ディレクトリ・ファイルの役割.md|
|公開前のチェックリスト|documents/公開前のチェックリスト.md|
...
### Q. URLの長いアイコンを複数登録したらアイコンが一部途切れる/出てこない。
...
さくらのレンタルサーバ等、一部のレンタルサーバでは.htaccess記載のSetEnvの値が無効になります。各phpファイルの`GETENV('GAME_ROOT').`を`$_SERVER['DOCUMENT_ROOT'].'/teiki'.`に置換するなどで対応を行ってください。変更ファイル数が多いため、マルチファイルを置換できるものがあると楽です。
